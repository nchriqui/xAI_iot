...
a = int(len(df) / 30)
ddd_ADWIN = window_average(stream_record_ADWIN, a)

x = np.arange(a, a * len(ddd_ADWIN) + 1, a)
...
correlation, p_value = pearsonr(ddd_ADWIN, trimmed_hellinger_distances)
print(f"Correlation between Accuracy and Hellinger Distance: {correlation}, p-value: {p_value}")

correlation, p_value = pearsonr(ddd_ADWIN, trimmed_kl_divs)
...
ax1.plot(x, ddd_ADWIN, 'r', label='ADWIN Accuracy', marker="*")
...
ax1.plot(x, ddd_ADWIN, 'r', label='D3 Accuracy', marker="*")
ax1.set_xlabel('Processed Data Points')
ax1.set_ylabel('Accuracy', color='r')
ax1.tick_params(axis='y', labelcolor='r')
